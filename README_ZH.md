...
- **OWNER_ID** : The Telegram user ID (not username) of the owner of the bot. @userinfobot
...
```
                         需要输入上面的API_KEY 和API_HASH 还要输入上面的bot_token
- **MEGA_API_KEY**: Mega.nz api key to mirror mega.nz links. Get it from [Mega SDK Page](https://mega.nz/sdk)   //如果没有账号可以不用填写
- **MEGA_EMAIL_ID**: Your email id you used to sign up on mega.nz for using premium accounts (Leave th)   //如果没有账号可以不用填写
- **MEGA_PASSWORD**: Your password for your mega.nz account   //如果没有账号可以不用填写
...
## Getting Google OAuth API credential file  获取验证文件，用来对google api进行操作
...
sudo dockerd
```
- Build Docker image: 主要要在mirror-bot文件夹下
```
...
```
  docker rm 容器id
```
- 停止docker，docker启动后在窗口下无法停止，需要新开一个命令窗口
- 列出所有容器 ID
```
   docker ps -aq
```
...
# Using service accounts for uploading to avoid user rate limit   突破上传限制
...
### 如果已经有accounts文件夹，可以将文件夹复制到mirror-bot文件夹下面
```
   cp -rf 源文件夹 目标文件夹
```
- 对accounts文件夹里面的文件进行重命名,在./mirror-bot/accounts/下面
```
   批量重命名

  i=0; for x in *; do mv $x $i.json;let i=i+1; done
